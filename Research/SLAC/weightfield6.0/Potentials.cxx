...
void Potentials::SetPitchWidthXY(void *wfgui, int dimy, int nstrip,double p, double w, double Stepx, double Stepy )
{
 ...
 WFGUI* gui= (WFGUI*) wfgui; 
  ...
  double limit = 0.03;
  if ( gui->GetPrecisionGain()) limit = 0.015;
  double dd = 0;
  double ratio = 0;
  if ( gui->GetGainon())
    {
      for(int y=0;y<YMAX;y++)	 
	if  (y*BinSizey  >= gui->Getygainlayerlow())
	  if (y*BinSizey  <  (gui->Getygainlayerhigh() ))
	    {
	      dd += BinSizey;
	    }
      
      if (dd !=0)  ratio = dd/(gui->Getygainlayerhigh() -   gui->Getygainlayerlow());
      //    cout << "High precision : " << gui->GetPrecisionGain() << endl;
      cout << "Actual gain layer thickness: " << dd << " Requested = " << gui->Getygainlayerhigh() -   gui->Getygainlayerlow()<<  " Ratio = " << ratio << endl;
      
      if (ratio < 1.-limit || ratio > 1.+limit)
	{
	  //	  multig ++ ;
	  cout << "y bin too large: " << BinSizey << " recalculating potential grid. # of multigrid: " << multig <<  endl;
	  Stepy = Stepy/2.;
	  Stepx  = Stepx/2.; 
	  goto MULTI;
	}
 ...
 //here we change from micron to bins
  ...
  //  gui->SetStepx(BinSizex);
  // gui->SetStepy(BinSizey);
...
{				// method for iterative calculation, see: http://en.wikipedia.org/wiki/Relaxation_%28iterative_method%29
  ...
  //  float binx =  gui->GetXMax()/XMAX;// gui->GetStepx()*1.*finestX/XMAX;
  // float biny =   gui->GetYMax()/YMAX;//gui->GetStepy()*1.*finestY/YMAX;

  float binx =  GetBinSizex()*1.*finestX/XMAX;
  float biny =  GetBinSizey()*1.*finestY/YMAX;
  
  //  biny = gui->GetYMax()/YMAX;
  double fixconst = 1;// 0.75; // needed to get the right depletion voltage (?)

  double h2 =  fixconst*biny*biny*1e-12; // matrix square, needed to calculate  the iterative steps

  //    cout <<  "Calculating grid with bin (x,y) = " << binx <<  ", " << biny << endl;
  ...
  //  cout << "ymin = " << ymin << " " << ymaxbd<<  " h2 (dopyx[1][1])" << h2 << " " << dopyx[1][1] <<  " [N/m3] h2 " << h2 << endl;
  // cout << "XMAX = " << XMAX << " " << ymaxbd<<  " h2 (dopyx[1][1])" << h2 << " " << dopyx[1][1] <<  " [N/m3] h2 " << h2 << endl;
  // int cco = 1;
  while(1)		// calculate until accuracy is reached
    {
      if(gui->Getstopped()==1) break;
      // cout << " cco= " << cco << endl;
      //  cco++;
	  ...
	  for (int y=ymin; y<(ymaxbd-1); y++)
	      ...
	      if (!tempfix[y][x])
		{
		  ...
		  if (gui->CallGetDetType() == 0 || gui->CallGetDetType() == 2)
		    {
		      // different weight in x-y
		      newpot.dpot[y][x]=(1./biny*dpot[y+1][x]+ 1./biny*dpot[y-1][x]+ 1./binx*dpot[y][(x+1+XMAX)%(XMAX)]+
					   1./binx*dpot[y][(x-1+XMAX)%(XMAX)])/(2./(biny)+2./(binx))+0.25*h2*(dopyx[y][x])*ECHARGE/(EPSILON*EPSILONR);
      ...
      if(it%100==0) // check every 1 00 iterations wether break condition fulfilled
	{
	  ...
	  for(int i=1; i<XMAX; i++) {		// calculate sum
	    for(int j=ymin; j<YMAX; j++) {
	      ...
	      //      cout << dpot[j][i] << " dpot " << newpot.dpot[j][i] <<   endl;
	    }
	  }

	  //	  cout << " err " << sum/((double)(XMAX*YMAX)) << " sum " << sum << " XMAX*YMAX " << XMAX << " " << YMAX <<  endl;
  ...
  //  for (int y=0; y<YMAX; y++) cout << "Pot in = " << y << " = " << dpot[y][10] << endl;
...
{				// method to set the doping profile in dopyx
  ...
  if (gui->GetDopinggainlayerValue() !=0 ) GainLayerThick =   gui->Getygainlayerhigh() - gui->Getygainlayerlow();
  ...
  for (int x=0; x<XMAX; x++)
      ...
      for (int y=0; y<YMAX; y++)
	{
	  ...
	  //	  if ( x == 10) cout << " ypos = " << y << " poissonf = " << poissonf*DopingRem << endl;
...
void Potentials::Restriktor()    //method to restrict potentials to a coarser grid, with XMAX/2+1
{
  ...
  for(int i=0;i<YMAX/2+1;i++)	// copy mutual points from 'old' potential to new potential
	...
	{
	  //	  cout  << " i j z s " << YMAX/2+1 << " " << XMAX/2+1 << " " <<  i << " " << j << " " << YMAX << " " << XMAX << " " << z << " " << s << endl;
	  if (z<YMAX && s<XMAX)
	    {
...
//   tempfix=FixRestriktor();	// set fix matrix
  ...
  for(int i=0;i<YMAX;i=i+2)    // copy mutual points from coarser grid to finer grid
      ...
      for(int j=0; j<XMAX;j=j+2)
	{
	  newpot.dpot[i][j]=dpot[z][s];
	  newpot.wpot[i][j]=wpot[z][s];
	  //  if (j ==0) cout << " dpotdpot[z][s]  "<< newpot.dpot[i][j]<<  " y = " << i << endl; 
	  s++;
	}
      z++;
    }
  ...
  for(int i=0;i<YMAX;i++) {			// overwriting old dpot and wpot with new ones
    for(int j=0;j<XMAX;j++) {
      wpot[i][j]=newpot.wpot[i][j];
      dpot[i][j]=newpot.dpot[i][j];
      //if (j ==0) cout << " dpotdpot[z][s]  "<< dpot[i][j]<<  " y = " << i << endl; 
    }
  }
...
void Potentials::Multigrid(void *wfgui, bool ReadOutTopFlag)
{
  ...
  // cout << "SetDopingProfile " << endl;
  SetDopingProfile(gui,0);

  //  cout << "Iteration " << endl;
  //Iteration(gui);

  cout << "Calculating Potentials, # of multigrids: " << multig << endl;
  ...
  for(int i=0; i<multig; i++) { 
    //sprintf(gridlabel,
    //    if (i ==0)  cout << "Calculating Potentials: grid number " << i+1 << "/" << multig;
    //  else cout << ", " << i+1 << "/" << multig;
    // sprintf(gridlabel2, "Calculating Potentials: grid number: %d/%d",i,multig+1);	
  ...
  // for(int y=0;y<YMAX;y++)
  // {
  //   cout << " dpot[y][10] "  << y << " " << dpot[y][10] << endl;
  // }
  ...
  if (gui->GetGainShape() !=0 && VFROMDOP != 1) // add gain  layer by hand
    {
      ...
      double dd = 0;
      double GIUpperEdge = 0;
      double GLDriftDistance = 0;
      double RatioDriftDistance = 1;
	...
	{	  
	 ...
	 if (gui->GetGainShape() != 0)  cout << "Fraction of inital acceptor in the gain implant still present = " << DopingRem <<
      ...
      if (gui->GetGainShape() != 0) cout << "Requested gain implant region: " << setprecision(4) << gui->Getygainlayerlow() << " - " << gui->Getygainlayerhigh() << " micron \n";
	  ...
	  if  (y*GetBinSizey()  >= gui->Getygainlayerlow())
		...
		{
		  dd += GetBinSizey();
		  GIUpperEdge = y*GetBinSizey();
		  
		}
      if  (bulk == PTYPE && strips == NTYPE )
	{
	  GLDriftDistance = YMAX*GetBinSizey() - ELECTRODE_DEPTH- GIUpperEdge;
	  RatioDriftDistance = (YMAX*GetBinSizey() - ELECTRODE_DEPTH-gui->Getygainlayerhigh())/GLDriftDistance;
	}
	//      DopScale = pow((gui->Getygainlayerhigh() -   gui->Getygainlayerlow())/dd,2);
	DopScale = (gui->Getygainlayerhigh() -   gui->Getygainlayerlow())/dd*RatioDriftDistance;
     
	cout << "Actual gain implant thickness: " << dd << " Requested = " << gui->Getygainlayerhigh() -   gui->Getygainlayerlow()<< endl;
	cout << "Actual gain implant upper position: " <<   GIUpperEdge << " Requested = " << gui->Getygainlayerhigh()  << endl;
	cout << "Actual gain drift length: " << GLDriftDistance <<  " Requested = " << YMAX*GetBinSizey() - ELECTRODE_DEPTH-gui->Getygainlayerhigh()  << endl;
	cout << "Doping scaled by = " << DopScale << endl;
	dd = 0;	      	

	Double_t Decrease[1000];
	Double_t Dec;
	Int_t NDecrease = 0;
	Double_t DDD;
      ...
      if  ( (bulk == PTYPE && strips == NTYPE ) || (bulk == NTYPE && strips == PTYPE ))
	{
	  for(int y=0;y<YMAX;y++)
	      ...
	      if  (y*GetBinSizey()  >= gui->Getygainlayerlow())
		{
	      ...
	      if (gui->GetGainShape()==10)
		{
		  if (y*GetBinSizey()< gui->Getygainlayerhigh()+MONODOPDIST && (y*GetBinSizey()>gui->Getygainlayerlow()))
		    {				      							      
		      NDecrease ++;
		      Decrease[NDecrease]=  DD+ddout;
		      Dec =  Decrease[NDecrease];
		      //    cout << y*GetBinSizey() << " pippo " <<  NDecrease << endl;
		    }
		  else
		    {
		      NDecrease--;
		      if (NDecrease < 1) NDecrease =1;
		      Dec =  - Decrease[NDecrease];
		    }
		}
	      ...
	      for(int x=0; x<XMAX;x++)
		{
		  if (bulk == PTYPE && strips == NTYPE )
		      ...
		      if (y !=YMAX-1)
			{
			  ...
			  else // no gain recess in Monolith
			    {
			      dpot[y][x] += VdepGainLayer*Dec;
			      if (y*GetBinSizey()  <  gui->Getygainlayerhigh() )
				{
				  dopyx[y][x] +=  LocalDop*DDop;
				  DDD = LocalDop*DDop;
				}
			      else if (y*GetBinSizey()  >  gui->Getygainlayerhigh() + MONODOPDIST && y*GetBinSizey()  <  gui->Getygainlayerhigh()+MONODOPDIST + gui->Getygainlayerhigh()-gui->Getygainlayerlow())
				{
				dopyx[y][x] -=  DDD;
				}
			    }
  ...
  cout << "Done Potential rescaling " << endl;
